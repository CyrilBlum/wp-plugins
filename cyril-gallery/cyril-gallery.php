...
add_action('wp_footer', function () {
    ...
    echo '<style>.blum-sub-filters{display:none;order:2;flex:0 0 100%;width:100%;gap:6px;flex-wrap:wrap;margin-top:10px;padding:8px 0 2px;border-top:1px solid #d7d8d0}.blum-sub-filters.is-visible{display:flex}.blum-sub-filters button{font-size:.9em}</style><script>(function(){document.querySelectorAll(".cyril-gallery").forEach(function(g){var groups=' . wp_json_encode(blum_gallery_settings(get_the_ID())['taxonomy']) . ',filters=g.querySelector(".cyril-gallery__filters"),buttons=[...filters.querySelectorAll("button")],figures=[...g.querySelectorAll("figure")],open=null;Object.keys(groups).forEach(function(parent){var children=buttons.filter(function(b){return groups[parent].indexOf(b.textContent.trim())>-1});if(!children.length)return;var old=buttons.find(function(b){return b.textContent.trim()===parent});if(old)old.remove();var sub=document.createElement("div");sub.className="blum-sub-filters";children.forEach(function(b){sub.appendChild(b)});filters.appendChild(sub);var main=document.createElement("button");main.textContent=parent;main.dataset.filter=parent;filters.insertBefore(main,sub);function apply(tag){figures.forEach(function(f){var tags=(f.dataset.tags||"").split("|");f.hidden=tag!=="all"&&((tag===parent&&children.every(function(c){return tags.indexOf(c.textContent.trim())<0}))||(tag!==parent&&tags.indexOf(tag)<0))})}main.onclick=function(){if(open&&open!==sub)open.classList.remove("is-visible");open=sub;sub.classList.add("is-visible");apply(parent)};children.forEach(function(b){b.onclick=function(){if(open&&open!==sub)open.classList.remove("is-visible");open=sub;sub.classList.add("is-visible");apply(b.textContent.trim())}});var all=g.querySelector("[data-filter=all]");if(all)all.onclick=function(){if(open)open.classList.remove("is-visible");open=null;apply("all")}})})})();</script>';
});
